...
(defrecord CachedBuilder [pre-render-context environment]
  ...
  (build-renderer [this file-path template-resolver renderer sanitizer error-handler]
    (let [{:keys [filters sanitizers dictionary json-serializer yaml-serializer fragment]} environment
          pre-render? (not (empty? pre-render-context))]
      (if pre-render?
        (let [template (parser/parse-template file-path template-resolver filters sanitizers dictionary sanitizer json-serializer yaml-serializer fragment)
