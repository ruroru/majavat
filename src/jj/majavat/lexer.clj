    ...
    (let [current-char (first my-sequence)
      ...
      (cond
        ...
        (cond
          ...
          (recur (rrest my-sequence) "" (conj vector {:type :opening-bracket}))
          :else
          (recur (rrest my-sequence) "" (conj vector {:type :text :value current-string} {:type :opening-bracket})))

        (and (= current-char \{) (= next-char \%))
        (cond
          (string/blank? current-string)
          (recur (rrest my-sequence) "" (conj vector {:type :block-start}))
          :else
          (recur (rrest my-sequence) "" (conj vector {:type :text :value current-string} {:type :block-start})))

        (and (= current-char \%) (= next-char \}))
        (let [trimmed-string (string/trim current-string)]
          (cond
            (= "endfor" trimmed-string)
            (recur (rrest my-sequence) "" (conj vector {:type :end-for} {:type :block-end}))

            (= "endif" trimmed-string)
            (recur (rrest my-sequence) "" (conj vector {:type :keyword-endif} {:type :block-end}))

            (= "else" trimmed-string)
            (recur (rrest my-sequence) "" (conj vector {:type :keyword-else} {:type :block-end}))

            (and (= "" trimmed-string) (= (:type (last vector)) :keyword-extends))
            (recur (rrest my-sequence) "" (conj vector {:type :block-end}))
        ...
        (recur (rest my-sequence) "" (conj vector {:type :identifier :value (mapv keyword (-> (string/trim current-string)
                                                                                              (str/split #"\.")))}))
...
(defn tokenize [string]
  (tokenize-recursively (seq string) "" []))