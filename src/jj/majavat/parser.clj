...
(defn- parse-ast
  ([lexed-list list current-block template-resolver filter-map]
   (parse-ast lexed-list list current-block false nil template-resolver filter-map []))
  ([lexed-list list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack]
   (if (empty? lexed-list)
     (if parsing-for-body
     ...
     (let [current-item (first lexed-list)]
       (case (:type current-item)
         :text (recur (rest lexed-list) (conj list current-item) current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         ...
         :expression (let [remaining (rest lexed-list)
                       ...
                       (recur remaining-after-filters (conj list value-node) current-block parsing-for-body current-file-path template-resolver filter-map tag-stack))

         :opening-bracket (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         ...
         :block-start (let [remaining (rest lexed-list)]
                        (if (and (seq remaining) (= :block-end (:type (first remaining))))
                          (let [remaining-after-block-end (rest remaining)]
                            (recur remaining-after-block-end list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack))
                          (recur remaining list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)))
         :block-end (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         ...
         :now (let [remaining (rest lexed-list)
                ...
                (recur final-remaining (conj list now-node) current-block parsing-for-body current-file-path template-resolver filter-map tag-stack))
         ...
         :keyword-csrf-token (recur (rest lexed-list) (apply conj list '({:type :text :value "<input type=\"hidden\" name=\"csrf_token\" value=\""}
                                                                         ...
                                                                         {:type :text :value "\">"}))
                                    current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
                                   ...
                                   (let [block-end-token (first remaining-after-decl)]
                                     (if (and block-end-token (= :block-end (:type block-end-token)))
                                       (let [remaining-after-block-end (rest remaining-after-decl)
                                         ...
                                         (recur remaining-after-block-end (conj list query-string-node) current-block parsing-for-body current-file-path template-resolver filter-map tag-stack))
                              ...
                              (let [filename (:value file-name-token)
                                ...
                                (if (cr/template-exists? template-resolver resolved-filename)
                                  (let [file-content (read-content-as-string template-resolver resolved-filename)
                                        ...
                                        included-content (parse-ast included-lexed [] {} false resolved-filename template-resolver filter-map [])]
                                    (recur remaining-after-filename (into list included-content) current-block parsing-for-body current-file-path template-resolver filter-map tag-stack))
                              ...
                              (let [file-path (:value file-path-token)
                                ...
                                (if (cr/template-exists? template-resolver resolved-file-path)
                                  (let [parent-content-str (read-content-as-string template-resolver resolved-file-path)
                                        ...
                                        [block-content remaining-after-block new-tag-stack] (parse-ast remaining-after-file-path [] current-block true current-file-path template-resolver filter-map tag-stack)
                                        parent-content (parse-ast parent-lexed [] {:content block-content} false resolved-file-path template-resolver filter-map [])]
                                    (recur remaining-after-block (into parent-content list) current-block parsing-for-body current-file-path template-resolver filter-map new-tag-stack))
         ...
         :keyword-block (let [remaining (rest lexed-list)
                          ...
                          (if replacement-content
                            (recur remaining-after-block-name (into list replacement-content) current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
                            (let [[block-content remaining-after-block updated-tag-stack] (parse-ast remaining-after-block-name [] current-block true current-file-path template-resolver filter-map tag-stack)]
                              (recur remaining-after-block (into list block-content) current-block parsing-for-body current-file-path template-resolver filter-map tag-stack))))
                            ...
                            (if (and block-end-token (= :block-end (:type block-end-token)))
                              (let [remaining-after-block-end (rest remaining-after-var-decl)
                                    ...
                                    let-node {:type           node-type
                                              ...
                                              :body           body}]
                                (recur remaining-after-body (conj list let-node) current-block parsing-for-body current-file-path template-resolver filter-map updated-tag-stack))
                                ...
                                (if (some? (:value source-token))
                                  (let [remaining-after-block-end (rest remaining-after-source)
                                        ...
                                        for-node {:type       :for
                                                  ...
                                                  :body       body}]
                                    (recur remaining-after-body (conj list for-node) current-block parsing-for-body current-file-path template-resolver filter-map updated-tag-stack))
                               ...
                               (if (some? (:value source-token))
                                 (let [remaining-after-block-end (rest remaining-after-source)
                                       ...
                                       each-node {:type       :each
                                                  ...
                                                  :source     (:value source-token)
                                                  :body       body}]
                                   (recur remaining-after-body (conj list each-node) current-block parsing-for-body current-file-path template-resolver filter-map updated-tag-stack))
                                 (throw (ex-info (format "error on line %s" (:line block-end-token))
                                                 {:type :syntax-error
                                                  :line (:line block-end-token)}))))
                             (throw (ex-info (format "error on line %s" (:line (first remaining-after-id)))
         ...
         :keyword-if (let [remaining (rest lexed-list)
                           ...
                           [when-true remaining-after-true updated-tag-stack-1] (parse-ast remaining-after-block-end [] current-block true current-file-path template-resolver filter-map new-tag-stack)
                           [when-false remaining-final updated-tag-stack-2] (if (and (seq remaining-after-true)
                                                                              ...
                                                                              (let [remaining-after-else (rest (rest remaining-after-true))
                                                                                    [else-body remaining-after-else-body updated-stack] (parse-ast remaining-after-else [] current-block true current-file-path template-resolver filter-map updated-tag-stack-1)]
                       ...
                       (if (some? (:value condition-token))
                         (recur remaining-final (conj list if-node) current-block parsing-for-body current-file-path template-resolver filter-map updated-tag-stack-2)
         ...
         :keyword-if-not (let [remaining (rest lexed-list)
                               ...
                               if-not-node {:type       :if
                                            ...
                                            :when-false when-false}]
                           (if (some? (:value condition-token))
                             (recur remaining-final (conj list if-not-node) current-block parsing-for-body current-file-path template-resolver filter-map updated-tag-stack-2)
                             (throw (ex-info (format "error on line %s" (:line condition-token))
                                             {:type :syntax-error
                                              :line (:line condition-token)}))))
         ...
         :keyword-in (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :each-in-token (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :each-identifier-token (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :variable-declaration (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :identifier (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :file-path (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :block-name (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :query-string-declaration (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :filter-tag (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :filter-function (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
         :filter-arg (recur (rest lexed-list) list current-block parsing-for-body current-file-path template-resolver filter-map tag-stack)
...
(defn parse
  ...
  (if (cr/template-exists? template-resolver resource-path)
    (let [file-content (read-content-as-string template-resolver resource-path)
      ...
      (try
        (parse-ast lexed-value [] {} false resource-path template-resolver filter-map [])
