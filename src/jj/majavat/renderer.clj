    ...
    (case (:type node)
      ...
      (let [condition (:condition node)
        ...
        (if (evaluate-condition condition context)
          (when (seq when-true) (render-nodes when-true context sb))
          (when (seq when-false) (render-nodes when-false context sb))))
...
(defn render-is
  ([template context]
   (render-is template context StandardCharsets/UTF_8))
